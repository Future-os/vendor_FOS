...
func init() {
	android.RegisterModuleType("future_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = futureExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = futureExpandVariables(ctx, dep_file)
	...
	cmd := futureExpandVariables(ctx, String(g.properties.Cmd))
