...
function setup_vendor() {
    ...
    export FUTURE_ROOT="$3"
    if [ ! -d "$FUTURE_ROOT" ]; then
        echo "\$FUTURE_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$FUTURE_ROOT/$OUTDIR" ]; then
        mkdir -p "$FUTURE_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$FUTURE_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDMK="$FUTURE_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$FUTURE_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function write_header() {
    ...
    if [[ $INITIAL_COPYRIGHT_YEAR =~ $NUM_REGEX ]] && [ $INITIAL_COPYRIGHT_YEAR -le $YEAR ]; then
        ...
        if [ $YEAR -eq 2017 ]; then
            printf "# Copyright (C) 2017 The FutureOS Project\n" >> $1
        elif [ $INITIAL_COPYRIGHT_YEAR -eq $YEAR ]; then
            printf "# Copyright (C) $YEAR The FutureOS Project\n" >> $1
        elif [ $INITIAL_COPYRIGHT_YEAR -le 2017 ]; then
            printf "# Copyright (C) 2017-$YEAR The FutureOS Project\n" >> $1
        else
            printf "# Copyright (C) $INITIAL_COPYRIGHT_YEAR-$YEAR The FutureOS Project\n" >> $1
        fi
    else
        printf "# Copyright (C) $YEAR The FutureOS Project\n" > $1
    fi
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$FUTURE_ROOT"/vendor/future/build/tools/smali/baksmali.jar
        export SMALIJAR="$FUTURE_ROOT"/vendor/future/build/tools/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$FUTURE_ROOT"/vendor/future/build/tools/"$HOST"/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$FUTURE_ROOT"/vendor/future/build/tools/"$HOST"/compact_dex_converter
    fi
    ...
    if [ ! -f "$FUTURE_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$FUTURE_TARGET" >/dev/null; then
...
function extract() {
    ...
    local SRC="$2"
    local OUTPUT_ROOT="$FUTURE_ROOT"/"$OUTDIR"/proprietary
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$FUTURE_ROOT"/"$OUTDIR"/radio
