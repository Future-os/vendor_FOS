function __print_future_functions_help() {
cat <<EOF
Additional Future-OS functions:
...
- githubremote:    Add git remote for Future-OS Github.
...
function breakfast()
{
    target=$1
    local variant=$2
    FUTURE_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/future/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the Future model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi

            lunch future_$target-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/future-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        if (adb shell getprop ro.future.device | grep -q "$FUTURE_BUILD"); then
...
EOF
            ...
            echo "The connected device does not appear to be $FUTURE_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function githubremote()
{
    ...
    git remote add github https://github.com/Future-OS/$PROJECT
...
function installboot()
{
    ...
    if (adb shell getprop ro.future.device | grep -q "$FUTURE_BUILD");
    then
...
function makerecipe() {
    ...
    then
        pwd
        futureremote
        git push future HEAD:refs/heads/'$1'
    fi
    '
}
...
function dopush()
{
    ...
    if (adb shell getprop ro.future.device | grep -q "$FUTURE_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $FUTURE_FIXUP_COMMON_OUT ]; then
